...
def test_preprocessor_output_shape(tmp_path):
    df = generate_dataset(n=200, seed=1)
    data_path = tmp_path / "dlp.csv"
    df.to_csv(data_path, index=False)
    X_train, X_test, y_train, y_test = load_and_split(str(data_path), test_size=0.2, random_state=1)
...
def test_train_test_split_is_stratified(tmp_path):
    df = generate_dataset(n=500, seed=1)
    data_path = tmp_path / "dlp2.csv"
    df.to_csv(data_path, index=False)
    X_train, X_test, y_train, y_test = load_and_split(str(data_path), test_size=0.2, random_state=1)
...
def test_alerts_reference_valid_users(tmp_path):
    users = generate_users(n=20, seed=1)
    users_path = tmp_path / "users.csv"
    users.to_csv(users_path, index=False)
    alerts = generate_dataset(n=500, seed=1, users_path=str(users_path))
...
def test_alert_risk_concentrates_by_user(tmp_path):
    ...
    users_path = tmp_path / "users2.csv"
    users.to_csv(users_path, index=False)
    alerts = generate_dataset(n=3000, seed=3, users_path=str(users_path))
...
def test_build_dashboard_payload_structure(tmp_path):
    ...
    users_path = tmp_path / "users3.csv"
    users.to_csv(users_path, index=False)
    alerts = generate_dataset(n=2000, seed=5, users_path=str(users_path))
    alerts_path = tmp_path / "alerts3.csv"
    alerts.to_csv(alerts_path, index=False)

    payload = build_dashboard_payload(
        str(alerts_path), str(users_path),
