...
16. ✅ Select the best model by PR-AUC and persist it
...
32. ✅ Build `src/generate_users.py`: 100-user synthetic roster with
    stable per-user traits (department, device, tenure, watchlist
    status, baseline risk propensity).
33. ✅ Rewrite `src/generate_data.py` to scale to 100,000 rows
...
39. ✅ Validate dashboard: confirmed JSON field names match the JS
...
43. ✅ Execute every code cell in the notebook in sequence and embed
    ...
    exactly.
44. ✅ Draft `reports/non_technical/Non_Technical_Report.docx`: a
...
45. ✅ Visually QA the report by rendering it to PDF and reviewing
    ...
    to a clean 5 pages).
46. ✅ Validated the docx with the skill's `validate.py` (47 paragraphs,
    all validations passed).
47. ✅ Re-ran the full test suite after all changes (14/14 tests pass).
48. ✅ Updated `README.md` with a "Capstone deliverables" section and
    refreshed the repository structure diagram to include both new
    files.

## Phase 12 — Technical Methodology Document ✅
49. ✅ Drafted `reports/technical/Technical_Methodology_Models_and_Techniques.docx`:
    a deep, technically-detailed companion document explaining which
    models were used (Logistic Regression, Random Forest, optional
    XGBoost), the exact hyperparameters for each, the class-imbalance
    strategies (`class_weight` vs. SMOTE) and why both exist, the
    preprocessing pipeline, the evaluation methodology (including why
    PR-AUC is emphasized over ROC-AUC and how the auto-resolve
    threshold sweep works), and a section on approaches that were
    considered and explicitly rejected (unsupervised anomaly
    detection, deep learning, naive undersampling) with reasoning for
    each. Includes 4 embedded figures and a model comparison table
    with real metrics pulled from `reports/metrics.json`.
50. ✅ Discovered and worked around two real bugs in the `docx` npm
...
51. ✅ Fixed an inline markdown-style `**bold**` parser bug where the
    raw asterisks were rendering literally instead of as bold text
    (the helper only handled backtick code spans initially).
52. ✅ Fixed a table-header row that was splitting across a page
    ...
    fresh page.
53. ✅ Visually QA'd all 9 pages of the final PDF rendering, page by
    page, to confirm every fix actually took effect (bold text,
    code block line breaks, table headers, page breaks) rather than
    trusting the schema validator alone.
54. ✅ Re-ran the full test suite after all changes (14/14 tests still
    pass — this document doesn't touch the data/model pipeline).
55. ✅ Updated `README.md` with the third capstone-adjacent deliverable
    and refreshed the repository structure diagram and quickstart.

## Phase 10 — Publish to GitHub (remaining manual steps)
26. ⬜ Create a new GitHub repository (e.g., `dlp-predictive-modeling`).
27. ⬜ `git init`, `git add .`, `git commit -m "Initial commit: DLP alert
    risk classification project"`.
28. ⬜ `git remote add origin <your-repo-url>` and `git push -u origin
    main`.
29. ⬜ (Optional) Add a GitHub Actions workflow to run `pytest tests/`
    on every push/PR for CI.
30. ⬜ (Optional) Add badges to the README (build status, license,
    Python version) once CI is set up.
31. ⬜ (Optional) Re-run `train.py` with `xgboost` and
    `imbalanced-learn` installed locally to capture their results in
    the README's results table, since this sandbox couldn't install
    those two packages (no network access) — everything still runs
    correctly without them via graceful fallbacks, but installing them
    will likely improve PR-AUC further.
31a. ⬜ Open `dashboard/index.html` in a real browser (served over
    HTTP, see `docs/dashboard.md`) and visually confirm both views
    render as expected. This sandbox could not launch a headless
    Chrome instance to screenshot it (no Chrome binary, no network to
    install one), so the dashboard was validated by: serving it and
    confirming HTTP 200 on both files, cross-checking every JSON field
    name the JS reads against the actual generated `data.json`,
    simulating the render logic in Node against real data, checking
    JS syntax with `node --check`, and verifying HTML tag balance
    programmatically. A quick visual pass in an actual browser is the
    one remaining check before you publish.
31b. ⬜ Compare `reports/non_technical/Non_Technical_Report.docx`
